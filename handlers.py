...
class MediaSearchHandlers:
    ...
    def _register_handlers(self):
        ...
        self.app.add_handler(filters.command("ban") & filters.private, self.ban_command)
        self.app.add_handler(filters.command("unban") & filters.private, self.unban_command)
        self.app.add_handler(filters.command("broadcast") & filters.private, self.broadcast_command)
        self.app.add_handler(filters.command("index") & filters.private, self.index_command)
        self.app.add_handler(filters.command("delete") & filters.private, self.delete_command)
        self.app.add_handler(filters.command("logger") & filters.private, self.logger_command)
        self.app.add_handler(filters.command("total") & filters.private, self.total_command)
        self.app.add_handler(filters.command("channel") & filters.private, self.channel_command)
        
        # Inline handlers
        self.app.add_handler(filters.inline_query, self.inline_query_handler)
        self.app.add_handler(filters.chosen_inline_result, self.chosen_inline_result_handler)
        
        # Callback handlers
        self.app.add_handler(filters.callback_query, self.callback_query_handler)
        ...
        # Error handler
        self.app.add_handler(filters.all, self.global_error_handler, group=999)
...
• `@{me.username} song name | audio`"""
    ...
    async def _create_inline_result(self, index: int, file_data: Dict[str, Any]):
        ...
        try:
            ...
            if mime_type:
                content_text += f"📄 **Format:** {mime_type}\n"
            ...
            return InlineQueryResultDocument(
                id=f"file_{index}",
                title=file_name[:100],  # Telegram title limit
                description=description,
                document_url=telegram_link,
